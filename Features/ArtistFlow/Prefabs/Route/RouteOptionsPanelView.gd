extends GridContainer
class_name RouteOptionsPanelView


func setup(view_model: Dictionary) -> void:
	var options := Array(view_model.get("options", []))
	var slots := get_slots()
	for index in slots.size():
		if slots[index].has_method("setup"):
			slots[index].call("setup", Dictionary(options[index]) if index < options.size() else {})


func refresh(view_model: Dictionary) -> void:
	setup(view_model)


func get_slots() -> Array[Control]:
	return _control_children()


func _control_children() -> Array[Control]:
	var result: Array[Control] = []
	for child in get_children():
		if child is Control:
			result.append(child as Control)
	return result
