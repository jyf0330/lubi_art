...
const GRID_DATA := preload("res://Data/BattleScene2GridData.gd")
...
func _make_grid_view_model(layout: Dictionary) -> Dictionary:
	...
	if cell_size <= 0 or canvas_size.x <= 0 or canvas_size.y <= 0:
		push_error("BattleScene2 grid layout has invalid dimensions.")
		return {}
	if canvas_size.x % cell_size != 0 or canvas_size.y % cell_size != 0:
		push_error("BattleScene2 cell size must divide the canvas without gaps.")
		return {}
	...
	if board_size.x <= 0 or board_size.y <= 0 or board_size.x > total_cells.x or board_size.y > total_cells.y:
		push_error("BattleScene2 board size must fit inside the full grid.")
		return {}
	if (total_cells.x - board_size.x) % 2 != 0 or (total_cells.y - board_size.y) % 2 != 0:
		push_error("BattleScene2 board cannot be centered on whole grid cells.")
		return {}
