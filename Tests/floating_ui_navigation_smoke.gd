...
func _run() -> void:
	...
	var main_bg := current_scene.get_node("MainBG") as TextureRect
	if main_bg.texture == null or main_bg.texture.resource_path != "res://Battle/Images/battle_bg/bg3.png":
	...
	if background_frame == null or background_frame.texture == null:
		...
		quit(1)
		return
	if background_frame.texture.resource_path != "res://Battle/Images/battle_interface_frame_2.png":
	...
	var middle_controller := current_scene.get_node("MainBG/Containers/Middle")
	...
	current_scene._unhandled_input(cancel_event)
	await process_frame
	if not paused:
	...
	var settings_menu := current_scene.get_node_or_null(SETTINGS_MENU_NODE)
	...
	var start_button = current_scene.get_node("StartGameButton")
	...
	var restored_middle := current_scene.get_node("MainBG/Containers/Middle")
