...
const BATTLE_SCENE_2 := "res://Scenes/battle_scene_2.tscn"
...
func _run() -> void:
	var packed_scene := load(BATTLE_SCENE_2) as PackedScene
	if packed_scene == null:
		_fail("Unable to load battle_scene_2")
		return
	...
	if grid_view == null:
		_fail("battle_scene_2 has no BattleGridView prefab")
		return
	...
	print("Battle scene 2 grid smoke test passed.")
	scene.queue_free()
	await process_frame
	quit()
