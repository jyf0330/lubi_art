...
```text
执行的 smoke test：shared_pet_prefab_smoke、merge_animation_smoke、storage_auto_merge_smoke、storage_shop_bag_merge_destination_smoke、battle_integration_smoke、battle_editor_preview_smoke
测试结果：上述去重相关 smoke 通过；silver_frame_size_smoke 因现有场景缺少 CreatureFrame 失败，与资源路径无关
...
结论：PASS
阻塞项：无
下一步：提交时精确纳入 Shared 资源、新 .import、旧副本删除及引用更新，避免混入并行业务改动
```
